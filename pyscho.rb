  ...
  def best_hand
    ...
    (0..@cards_in_hand.length).each do |card_number_in_hand|
      @cards_in_hand.combination(card_number_in_hand).each do |combination|
        ...
        probable_best_hand = ('straight-flush' if is_straight? && is_flush?) || ('four-of-a-kind' if kinds? [4, 1]) || ('full-house' if kinds? [3, 2]) || ('flush' if is_flush?) || ('straight' if is_straight?) || ('three-of-a-kind' if kinds? [3, 1, 1]) || ('two-pairs' if kinds? [2, 2, 1]) || ('one-pair' if kinds? [2, 1, 1, 1]) || 'highest-card'
